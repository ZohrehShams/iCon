 ...
 * if X is related to exactly 3 Y's then we can conclude that it relates to at least 2 Y's. 
...
implements BasicInferenceRule<MultipleRuleArgs>, ForwardRule<MultipleRuleArgs>, Serializable{
    ...
    @Override
    public GeneraliseEqualityCardinality getInferenceRule() {
        return this;
    }
    ...
    public String getDescription(Locale locale) {
        return i18n(locale,"GENERALISE_EQUALITY_CARDINALITY_DESCRIPTION");
    }
    

    @Override
    public String getPrettyName(Locale locale) {
        return i18n(locale,"GENERALISE_EQUALITY_CARDINALITY_PRETTY_NAME");
    }
    ...
    private RuleApplicationResult apply(ArrayList<ArrowArg> targetArrows, Goals goals, boolean applyForward) throws RuleApplicationException {
        ...
        newSubgoals[inferenceTarget.getSubgoalIndex()] = targetSubgoal.transform(new GeneraliseEqualityCardinalityTransformer(inferenceTarget, applyForward));
