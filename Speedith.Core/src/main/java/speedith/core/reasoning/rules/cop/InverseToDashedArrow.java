...
/**
 * This rule operates on unitary diagrams. When there is a solid arrow with inverse label and no cardinality, sourced at a named spider (e.g., u)  
 * and targeted at an unnamed curve that contains a named spider (e.g., v), we can swap the inverse arrow with a dashed arrow from u to v. 
 * @author Zohreh Shams
 */
...
implements BasicInferenceRule<MultipleRuleArgs>, ForwardRule<MultipleRuleArgs>, Serializable{
	...
	public String getDescription(Locale locale) {
		return Translations.i18n(locale, "INVERSE_TO_DASHED_ARROW_DESCRIPTION");
	}
	
	
	@Override
	public String getPrettyName(Locale locale) {
		return Translations.i18n(locale, "INVERSE_TO_DASHED_ARROW_PRETTY_NAME");
	}
    ...
    private RuleApplicationResult apply(ArrowArg targetArrow, SpiderArg targetSpider, Goals goals) throws RuleApplicationException {
        ...
        newSubgoals[targetArrow.getSubgoalIndex()] = targetSubgoal.transform(new InverseToDashedArrowTransformer(targetArrow, targetSpider,true));
